...
object December3 {
  ...
  def main(args: Array[String]): Unit = {
    ...
    println(closestIntersectionPointDistance(d1, d2))
  }

  private def goRight(p: Point, n: Int): (Path, Point) = (0 to n).map((n: Int) => Point(p.x + n, p.y)).toSet -> Point(p.x + n, p.y)
  private def goLeft(p: Point, n: Int): (Path, Point) = (0 to n).map((n: Int) => Point(p.x - n, p.y)).toSet -> Point(p.x - n, p.y)
  private def goUp(p: Point, n: Int): (Path, Point) = (0 to n).map((n: Int) => Point(p.x, p.y + n)).toSet -> Point(p.x, p.y + n)
  private def goDown(p: Point, n: Int): (Path, Point) = (0 to n).map((n: Int) => Point(p.x, p.y - n)).toSet -> Point(p.x, p.y - n)
