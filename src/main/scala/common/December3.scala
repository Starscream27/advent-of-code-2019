...
object December3 {
  ...
  private def parsePathDirectives(directives: String) = {
    ...
    directives
      ...
      .toList
      .filter(_.nonEmpty)
      .map(parseSingleDirective)
  }

  private def movePointWithDirectives(point: Point, directives: String): Path = {
    var path = Array[Point]()
    var currentPoint = point
    parsePathDirectives(directives).foreach {
      case (move, distance) => {
        val (moved, lastPoint) = move(currentPoint, distance)
        currentPoint = lastPoint
        path = path ++ moved
      }
      case _ => println(s"Could not move point with directives $directives")
    }
    path
  }

  def closestIntersectionPointDistance(directives1: String, directives2: String) = {
    val path1 = movePointWithDirectives(Point(0, 0), directives1)
    val path2 = movePointWithDirectives(Point(0, 0), directives2)
    def calculateDistance(point: Point): Int = math.abs(point.x) + math.abs(point.y)

    (path1 intersect path2)
      .filterNot(p => p.x == 0 && p.y == 0) // remove start point
      .map(calculateDistance)
      .min
  }
  def shortestIntersectionPointDistance(directives1: String, directives2: String): Int = {
    val path1 = movePointWithDirectives(Point(0, 0), directives1)
    val path2 = movePointWithDirectives(Point(0, 0), directives2)

    def calculateDistance(point: Point): Int = {
      // index of each point is actually the number of steps taken to reach it (+ 1 to compensate 0 bound index)
      val stepsInPath1 = path1.indexOf(point) + 1
      val stepsInPath2 = path2.indexOf(point) + 1
      if (stepsInPath1 == 0 || stepsInPath2 == 0) throw new Error(s"Could not find point $point")
